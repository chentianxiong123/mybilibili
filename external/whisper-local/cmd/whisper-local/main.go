...
import (
	...
	"mybilibili/whisper_local/internal/whisper"
)

func main() {
	addr := getEnv("HTTP_ADDR", ":8095")
	...
	mux.HandleFunc("/api/v1/capabilities", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"engine":"whisper-cpp","model_dir":%q}`,
	...
	log.Printf("whisper-local listening on %s", addr)
