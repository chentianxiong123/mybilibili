...
import (
	...
	"mybilibili/whisper/internal/whisper"
)

func main() {
	addr := getEnv("HTTP_ADDR", ":8093")
	...
	svc := whisper.NewService(storage)

	mux := http.NewServeMux()
	whisper.RegisterRoutes(mux, svc)
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"status":"ok"}`))
	})
	mux.HandleFunc("/api/v1/capabilities", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprintf(w, `{"service":"whisper","backends":["openai-compatible","cloudflare"]}`)
	})
	...
	log.Printf("whisper service listening on %s", addr)
