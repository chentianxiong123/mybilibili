...
public class CreatorFansServiceImpl implements CreatorFansService {
    ...
    @Override
    public int getFansCount(Integer userId, Boolean mutual) {
        if (Boolean.TRUE.equals(mutual)) {
            // 统计互关粉丝数
            return userInteractionMapper.countMutualFollowers(
                    userId, TARGET_TYPE_USER, InteractionType.FOLLOW.getCode());
        } else {
            // 统计所有粉丝数
            return userInteractionMapper.countFollowersByTargetId(
                    userId, TARGET_TYPE_USER, InteractionType.FOLLOW.getCode());
        }
    }
