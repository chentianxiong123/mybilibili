...
public class VideoServiceImpl implements VideoService {
    ...
    private VideoVO buildVideoVO(Video video, Integer currentUserId) {
        ...
        // 设置上传者信息
        // 优先从稿件获取上传者ID，如果稿件不存在则使用视频的userId
        Integer uploaderId = video.getUserId();
        if (video.getManuscriptId() != null) {
            Manuscript manuscript = manuscriptMapper.selectById(video.getManuscriptId());
            if (manuscript != null && manuscript.getUserId() != null) {
                uploaderId = manuscript.getUserId();
            }
        }
        
        VideoVO.UserInfo userInfo = new VideoVO.UserInfo();
        com.mybilibili.common.entity.User user = userMapper.findById(uploaderId);
        System.out.println("【调试】buildVideoVO - 视频上传者ID: " + uploaderId + ", 当前用户ID: " + currentUserId);
        if (user != null) {
