...
export async function getVideoInfo(aId: number) {
  // 先返回本地缓存（若存在），再后台请求并覆盖
  const cached = readCache<any>('detail', String(aId))
  let result = null
  try {
    const res = await api.get(`/manuscript/${aId}`)
    const data = res?.data || res
    if (!data) {
      return cached ? { code: '1', data: cached, fromCache: true } : { code: '0', data: null }
    }
    ...
    console.error(e)
    // 网络失败时降级返回缓存
    if (cached) return { code: '1', data: cached, fromCache: true }
    if (result) return { code: '1', data: result }
