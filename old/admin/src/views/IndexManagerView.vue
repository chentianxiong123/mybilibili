...
const fetchIndexStatus = async () => {
  loading.value = true
  try {
    const response = await request.get('/api/search/admin/index/status')
...
const handleBulkIndex = async () => {
  try {
    ...
    const response = await request.post('/api/search/admin/index/bulk')
    if (response.code === 200) {
...
const handleRebuildIndex = async () => {
  try {
    ...
    actionLoading.value = true
    const response = await request.post('/api/search/admin/index/rebuild')
    if (response.code === 200) {
      ElMessage.success(response.message)
      setTimeout(fetchIndexStatus, 5000)
    } else {
...
const handleIncrementalIndex = async () => {
  try {
    ...
    actionLoading.value = true
    const response = await request.post('/api/search/admin/index/incremental?minutes=60')
    if (response.code === 200) {
      ElMessage.success(response.message)
