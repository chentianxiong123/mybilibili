...
const handleRestore = async (row) => {
  try {
    const res = await restoreContent(row.contentType, row.id)
    if (res.code === 200 || res.success) {
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await deleteContent(row.contentType, row.id)
    if (res.code === 200 || res.success) {
  ...
  <div class="content-review-page">
    ...
    <el-tabs v-model="activeTab" @tab-change="handleTabChange" class="review-tabs">
      <el-tab-pane label="待审核" name="pending">
        <template #label>
          <span>待审核</span>
          <el-badge v-if="total > 0" :value="total" class="tab-badge" />
        </template>
      </el-tab-pane>
    ...
    >
      ...
      <el-table-column label="类型" width="100">
        <template #default="{ row }">
          <el-tag :type="row.contentType === 'COMMENT' ? 'primary' : 'info'">
