...
const getTranscodeStatus = (row) => {
  ...
  // 检查是否所有视频都转码成功（processStatus === 11 表示转码成功）
  const allTranscoded = row.videos.every(video => video.processStatus === 11 || video.processStatus === 21 || video.processStatus === 31 || video.processStatus === 41 || video.processStatus === 5)
  
  if (allTranscoded) {
    return { text: '转码成功', type: 'success' }
  } else {
    return { text: '仍未完成转码', type: 'warning' }
  }
}
